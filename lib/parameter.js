...
function createRegExpFunction(re) {
  var msg = 'should match ' + re;
  return function regexpCheck(value) {
    return !re.test(value) ? msg : null;
  };
}

function createArrayFunction(arr) {
  var msg = 'should match one of ' + JSON.stringify(arr);
  return function arrayCheck(value) {
    return arr.indexOf(value) < 0 ? msg : null;
  };
}
  ...
  for (var key in rules) {
    ...
    if (!checkFunction) {
      ...
      if (ruleType === 'string') {
        ...
        // key: ['y', 'm', 'd']
        checkFunction = createArrayFunction(rule);
      } else {
        empty = rule.empty;
        if (rule.type) {
          ...
          if (ruleType === 'string') {
            ...
            // key: { type: ['y', 'm', 'd'] }
            checkFunction = createArrayFunction(rule.type);
          }
        } else if (rule.isDate) {
          // key: { isDate: true }
